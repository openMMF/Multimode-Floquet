...
\end{itemize} 
All options of the \verb make  command produces the static library \verb libopenmmf saving it in the folder \verb ~/lib . A number of \verb .mod files product of the building process are moved to the folder \verb ~/include  and required for running applications.
...
\noindent 
The variable \verb ID  contains information about the type of system, such as the number of levels and their energy spectrum (see the declaration of \verb TYPE(ATOM)  in Section \ref{sec:derivedtypes}).  The derived type \verb FIELDS  stores information required build the components of the Hamiltonian as well as the explicit matrix representation of the couplings.
...
The total number of driving frequencies is equal to the sum of all elements of the array \verb MODES_NUM  . The user then should allocate sufficient memory space to store each one of the matrix representation of the couplings $V^{\ell,n}$. This is done with the sequence of instructions:
...
By default the first element of the array of \verb FIELDS  is reserved for the static component of the Hamiltonian, which includes the spectrum of the static system as diagonal elements of the matrix \verb FILEDS(1)%V .  The next step then consist in defining all components of the Hamiltonian. For an quantum system with arbitrary energy spectrum, each one of the matrices \verb FIELDS(m)%V must be declared explicitly. 
...
\end{verbatim}
where the correspondence with eq. (\ref{eq:Vspinparametrisation}) is obvious. This set of instructions is repeated for each one of the driving fields, as can be seen in the source code.
...
We remind the user that when the system of interest is not one of the default types defined in section \ref{sec:InitOption}, the user must define explicitly and in full all the coupling matrices. For an example of this situation, see  the source file  \verb example/FORTRAN/main_lattice.f90 .

\subsection{Multimode Floquet matrix and diagonalisation}

Once the components of the Hamiltonian are defined (i.e the complete set of matrices \verb FIELDS(r)%V  has been initialise), the multimode Hamiltonian can be calcuated calling the function:
...
The variation of the CSR representation of the matrix is produced by sorting the array of ROW position, in such a way that all non-zero values of a given row become stored consequtively. This sorting is done using a \verb QUICK_SORT algorithm of the Numerical Reciepes book. Tor this to work properly for the present application,  the user must make sure that the internal arrays of the sorting function are big enough depending on the number of non-zero values. If the user notes that MKL eigenvalue calculator fails for matrices larger than certain dimension (but it works for smaller ones), it is possible that the  internal arrays of \verb QUICK_SORT algorithmmust be of bigger size. This can be fixed modifying the values of \verb NN and \verb NSTACK  of the function \verb QUICK_SORT_INTEGERS  located in the file \verb src/quick-sort-index-table.f90 . Alternatively, the user can produce the \verb value, \verb column, \verb \row  storage of the extended Hamiltonian matrix calling the function \verb MULTIMODEFLOQUETMATRIX_SP having defined \verb INFO=6 , and use other sparse eigenproblem solver.
...
The time evolution operator is stored in the complex matrix \verb U_AUX , whose size is equal to the number of bare states \verb ID%D_BARE .
...
The micromotion operator is stored in the square matrix \verb U_F1 , of size \verb ID%D_BARE .

\subsection{Identifying the dressing modes}

In several application is useful to define a dressed basis of states. The library includes functions to simplify the evaluation of the evolution operator in the basis of dressed states. For this, first the user should identify the subset of driving fields that define the dressed states. This is done using a integer array with as a many components as dressing fields. The elements of this array indicate the indices of the fields corresponding to the array \verb modes_num . For example, if there is only one dressing field corresponding to the second component of the array \verb MODES_NUM, then the array \verb DRESSINFIELDS_INDICES must be:

\begin{verbatim}
INTEGER, DIMENSION(2) :: DRESSINGFIELDS_INDICES
DRESSINGFIELDS_INDICES(1) = 1 ! THE STATIC COMPONEN
DRESSINGFIELDS_INDICES(2) = 2 ! THE FIRST DRIVING FIELD
\end{verbatim} 
...
The Fourier components are stored in the matrix \verb U_FD and the spectrum of dressed energies are stored in the array \verb E_DRESSED .  With these two elements, we can calculate the micromotion operator of the dressed basis using the subroutine:
...
The prototype of all function enabled for C++ are declared in the header file \verb include/MultimodeFloquet.h . This scheme let us to establish a line-by-line  correspondence between the Fortran and C++ source codes.

\section{Bugs and known limitations}

If you find any bug please contact the developing team using the github hosting link https://github.com/gsinuco/MultimodeFloquet/releases
