! THIS IS A DRIVER SUBROUTINE TO  SIMPLIFY THE SETTING UP OF A PROBLEM
! THE USE SHOULD ONLY DEFINE:
! NAME: OF THE SYSTEM
! MODES_NUM: an array with the number of modes and harmonics
! N_FLOQUET: The number of Floquet channels. Make sure that there is a default.
! ARRAYS: as any as the number of frequencies but whose form
!         depends on the system
! INFO: AN Error/option flag
!
! this routine the should define the data derived types needed to evaluate the 
! Floquet operator and etc, avoiding the user to do it explicitly.
!
MODULE FLOQUETINITINTERFACEDRIVER
  INTERFACE FLOQUETINITDRIVER
     MODULE PROCEDURE FLOQUETINITDRIVER_QUBIT, FLOQUETINITDRIVER_SPIN,FLOQUETINITDRIVER_ALKALI,FLOQUETINITDRIVER_MBH
  END INTERFACE FLOQUETINITDRIVER

CONTAINS

  SUBROUTINE FLOQUETINITDRIVER_QUBIT(NAME, MODES_NUM,N_FLOQUET,ARRAYS,INFO)
    USE FLOQUETINITINTERFACE
    USE ATOMIC_PROPERTIES
    USE TYPES    
    
    FLOQUETINIT_QUBIT(NAME,ARRAYS,INFO)
  END SUBROUTINE FLOQUETINITDRIVER_QUBIT


  SUBROUTINE FLOQUETINITDRIVER_SPIN(NAME, MODES_NUM,N_FLOQUET,ARRAYS,INFO)
    USE FLOQUETINITINTERFACE
    USE ATOMIC_PROPERTIES
    USE TYPES    
  END SUBROUTINE FLOQUETINITDRIVER_SPIN

  SUBROUTINE FLOQUETINITDRIVER_ALKALI(NAME, MODES_NUM,N_FLOQUET,ARRAYS,INFO)
    USE FLOQUETINITINTERFACE
    USE ATOMIC_PROPERTIES
    USE TYPES    
  END SUBROUTINE FLOQUETINITDRIVER_ALKALI

  SUBROUTINE FLOQUETINITDRIVER_MBH(NAME, MODES_NUM,N_FLOQUET,ARRAYS,INFO)
    USE FLOQUETINITINTERFACE
    USE ATOMIC_PROPERTIES
    USE TYPES    
  END SUBROUTINE FLOQUETINITDRIVER_MBH
  
  
END MODULE FLOQUETINITINTERFACEDRIVER
