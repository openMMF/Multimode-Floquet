...
total_frequencies_ = np.sum(modes_num) # // number of dressing fields
...
for r in range(N_):
    ...
    for m in range(M_):
      ...
      U_F1_red = np.reshape(U_F1_red,[d_bare,d_bare])
      U_F2_red = np.reshape(U_F2_red,[d_bare,d_bare])
      U_AUX    = np.reshape(U_AUX,[d_bare,d_bare])
      #print(np.power(np.abs(U_AUX),2))
      #print(np.power(np.abs(U_F1_red),2))
      #print(np.power(np.abs(U_F2_red),2))
      #print(U_AUX)
      #print(U_F1_red)
      #print(U_F2_red)
      ...
      U_T = np.reshape(U_T,[d_bare*d_bare])
